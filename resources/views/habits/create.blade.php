<x-app title="إضافة عادة جديدة">
    <div class="max-w-5xl mx-auto space-y-5 pb-24 sm:pb-16 px-4 sm:px-6" dir="rtl">
            ...
            <div class="relative flex flex-col sm:flex-row sm:items-center sm:justify-between gap-5">
                ...
                <div class="space-y-2">
                    <div class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-[#0D7371]/8 ring-1 ring-inset ring-[#0D7371]/12">
                        ...
                        <span class="text-[10px] font-medium text-[#0a5f5d] font-display">عادات جديدة</span>
                    </div>
                    ...
                    <p class="text-xs font-normal text-slate-400 max-w-md leading-relaxed">
                        صمم عادتك بدقة وحدد أهدافك لتضمن الاستمرارية والنجاح.
                    </p>
                </div>
        ...
        {{-- ==================== الفورم ==================== --}}
        <form action="{{ route('habits.store') }}" method="POST" class="space-y-5" id="habit-form" novalidate>
            @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            <select name="frequency_type" id="frequency_type"
                                ...
                                <option value="daily" {{ old('frequency_type') == 'daily' ? 'selected' : '' }}>يومي</option>
                                <option value="weekly" {{ old('frequency_type') == 'weekly' ? 'selected' : '' }}>أسبوعي</option>
                                <option value="monthly" {{ old('frequency_type') == 'monthly' ? 'selected' : '' }}>شهري</option>
                            </select>
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="number" name="frequency_target" id="frequency_target"
                            value="{{ old('frequency_target', 7) }}" min="1"
            ...
            <div class="relative overflow-hidden rounded-3xl border border-white/60 bg-white/60 backdrop-blur-2xl p-6 sm:p-7 shadow-[0_2px_16px_rgba(15,23,42,0.03)]">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div class="space-y-2 md:col-span-2">
                        ...
                        <div class="relative">
                            <select name="habit_type" id="habit_type"
                                ...
                                <option value="boolean" {{ old('habit_type', 'boolean') == 'boolean' ? 'selected' : '' }}>
                                    ✅ نعم / لا (إنجاز يومي)
                                </option>
                                <option value="numeric" {{ old('habit_type') == 'numeric' ? 'selected' : '' }}>
                    ...
                    <div id="numeric-fields" class="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="unit" id="unit"
                                value="{{ old('unit', 'مرة') }}" placeholder="صفحة، كوب..."
                ...
                <div class="relative overflow-hidden rounded-3xl border border-white/80 bg-white/95 backdrop-blur-2xl px-4 py-3 sm:px-5 sm:py-3.5 shadow-[0_12px_40px_-12px_rgba(13,115,113,0.25),0_4px_16px_-4px_rgba(15,23,42,0.08)]">
                    ...
                    <div class="relative flex flex-col-reverse sm:flex-row sm:items-center sm:justify-end gap-2.5">
                        ...
                        <button type="submit"
                            ...
                            <span class="relative font-display">حفظ العادة</span>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const placeholders = {
                daily: 'مثال: 1 أو 3',
                weekly: 'مثال: 5 أو 7',
                monthly: 'مثال: 20 أو 30',
            };

            function updateFrequencyHelper() {
                const type = frequencyTypeSelect.value;
                frequencyHelper.textContent = helpers[type] || '';
                frequencyTarget.placeholder = placeholders[type] || '';
            }
            frequencyTypeSelect.addEventListener('change', updateFrequencyHelper);
            updateFrequencyHelper();

            // العثور على أول خطأ وإجراء Scroll تلقائي إليه بعد إرسال النموذج وفشله
            const firstErrorElement = document.querySelector('.error-message');
            if (firstErrorElement) {
                const inputName = firstErrorElement.getAttribute('data-input');
                const targetInput = document.getElementById(inputName);
                if (targetInput) {
                    // فتح حقول الأرقام إذا كان الخطأ متعلقاً بها وهي مخفية
                    if ((inputName === 'target_value' || inputName === 'unit') && habitTypeSelect.value !== 'numeric') {
                        habitTypeSelect.value = 'numeric';
                        toggleNumericFields();
                    }
